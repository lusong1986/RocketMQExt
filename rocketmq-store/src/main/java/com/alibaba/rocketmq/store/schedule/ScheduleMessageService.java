...
public class ScheduleMessageService extends ConfigManager {
	...
	private void updateOffset(int delayLevel, long offset) {
		log.info(">>>>>>>>>>>>updateOffset delayLevel:" + delayLevel + ", offset:" + offset);
	...
	public void start() {
		...
		log.info(">>>>>>>>>>>>ScheduleMessageService start offsetTable:" + offsetTable);
		...
		for (Integer level : this.delayLevelTable.keySet()) {
			...
			offsetTable.put(3, 0L);

			if (timeDelay != null) {
				log.info(">>>>>>>>>>>>>>>>>ScheduleMessageService schedule >>>>>level: " + level + ",offset:" + offset);
	...
	class DeliverDelayedMessageTimerTask extends TimerTask {
		...
		public void executeOnTimeup() {
			...
			if (cq != null) {
				...
				if (bufferCQ != null) {
					log.info(">>>>>>>>>>>>>>>ScheduleMessageService.executeOnTimeup>>>>>>>>>>offset: " + offset + ","
							+ cq.getTopic() + ",queueId:" + cq.getQueueId() + ",bufferCQ:" + bufferCQ);
				}
				if (bufferCQ != null) {
					try {
						...
						for (; i < bufferCQ.getSize(); i += ConsumeQueue.CQStoreUnitSize) {
							...
							log.info(">>>>>>>>>>>>>>>ScheduleMessageService.executeOnTimeup>>>>>>>>>>offsetPy: "
									+ offsetPy + ",sizePy:" + sizePy + ",tagsCode:" + tagsCode);
							...
							if (countdown <= 0) {
								...
								log.info(">>>>>>>>>>>>>>>ScheduleMessageService.executeOnTimeup>>>>>>>>>>msgExt: "
										+ msgExt.toString());

								if (msgExt != null) {
									try {
										MessageExtBrokerInner msgInner = this.messageTimeup(msgExt);
										log.info(">>>>>>>>>>>>>>>ScheduleMessageService.executeOnTimeup>>>>>>>>>>new msgInner: "
												+ msgInner.toString());

										PutMessageResult putMessageResult = ScheduleMessageService.this.defaultMessageStore
												.putMessage(msgInner);

										log.info(">>>>>>>>>>>>>>>ScheduleMessageService.executeOnTimeup>>>>>>>>>>new putMessageResult: "
												+ putMessageResult.toString());
										// 成功
										...
										/*
										 * XXX: warn and notify me msgExt里面的内容不完整 ，如没有REAL_QID,REAL_TOPIC之类的

...
public class HAConnection {
	...
	public HAConnection(final HAService haService, final SocketChannel socketChannel) throws IOException {
		...
		log.info(">>>>>>>>>>>>>>HAConnection constrctor, haConnectionCount, this.haService.getConnectionCount():"
				+ this.haService.getConnectionCount().get());
	}
	...
	public void start() {
		log.info(">>>>>>>>>>>>>>HAConnection start socketChannel:" + JSON.toJSONString(socketChannel) + ",clientAddr:"
				+ clientAddr);
	...
	public void shutdown() {
		log.info(">>>>>>>>>>>>>>HAConnection >><>>>>>>>>>>>>>shutdown socketChannel:" + socketChannel + ",clientAddr:"
				+ clientAddr);
	...
	class ReadSocketService extends ServiceThread {
		...
		public void run() {
			HAConnection.log
					.info(">>>>>>>>>>>>>>ReadSocketService.run>>>" + this.getServiceName() + " service started");
			...
			while (!this.isStoped()) {
				try {
					...
					if (interval > HAConnection.this.haService.getDefaultMessageStore().getMessageStoreConfig()
						...
						log.warn(">>>>>>>>>>>>>>>>>>ReadSocketService.run>>>>ha housekeeping, found this connection["
								+ HAConnection.this.clientAddr + "] expired, " + interval);
						break;
					}
				} catch (Exception e) {
					HAConnection.log.error(">>>>>>>>>>>>>>>>>>ReadSocketService.run>>>>" + this.getServiceName()
			...
			HAConnection.log.info(">>>>>>>>>>>>>>>>>>ReadSocketService.run>>>>" + this.getServiceName()
					+ " service end");
		}
				...
				try {
					...
					if (readSize > 0) {
						...
						if ((this.byteBufferRead.position() - this.processPostion) >= 8) {
							...
							// log.info(">>>>>>>>>>>>>>>>>>>>>>>>ReadSocketService.processReadEvent>>>slave[" +
							// HAConnection.this.clientAddr +",processPostion:" +processPostion +",readSize:" +readSize
							...
							if (HAConnection.this.slaveRequestOffset < 0) {
								...
								log.info(">>>>>>>>>>>>>>>>>>>>>>>>ReadSocketService.processReadEvent>>>>>>>>slave["
										...
										+ ">>>slaveRequestOffset:" + HAConnection.this.slaveRequestOffset);
							}

							// log.info(">>>>>>>>>>>>>>>>>>>>>>>>ReadSocketService.processReadEvent>>>>>>>>call HAConnection.this.haService.notifyTransferSome to modify push2SlaveMaxOffset,HAConnection.this.slaveAckOffset:"
	...
	class WriteSocketService extends ServiceThread {
		...
		public WriteSocketService(final SocketChannel socketChannel) throws IOException {
			...
			log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService constructor>>>>>>>>socketChannel:"
					+ JSON.toJSONString(this.socketChannel));
		}

		@Override
		public void run() {
			HAConnection.log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService.run>>" + this.getServiceName()
					+ " service started");
...
//						HAConnection.log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService.run>>nextTransferFromWhere:"
...
//					}
					...
					if (-1 == this.nextTransferFromWhere) {
						...
						log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService.run>>master transfer data from "
					...
					if (selectResult != null) {
						log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService.run>>selectResult:]"
								+ selectResult.getStartOffset() + "," + selectResult.getSize()
						...
						this.lastWriteOver = this.transferData();
						log.info(">>>>>>>>>>>>>>>>>>>>>>>>WriteSocketService.run>>after transferData.thisOffset:"
								+ thisOffset + ", size:" + size + ", lastWriteOver:" + lastWriteOver);
					} else {
						// 没有数据，等待通知

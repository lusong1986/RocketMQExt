...
public class CommitLog {
	...
	public CommitLog(final DefaultMessageStore defaultMessageStore) {
		log.info(">>>>>>>>>>>>>>>>>CommitLog constructor :");
	...
	public boolean load() {
		...
		log.info(">>>>>>>>>>>>>>>>CommitLog load commit log " + (result ? "OK" : "Failed"));
		return result;
	}
	...
	public void recoverNormally() {
		log.info(">>>>>>>>>>>>>>>CommitLog.recoverNormally>>>>>>>>");
	...
	public DispatchRequest checkMessageAndReturnSize(java.nio.ByteBuffer byteBuffer, final boolean checkCRC,
		...
		try {
			java.nio.ByteBuffer byteBufferMessage = ((DefaultAppendMessageCallback) this.appendMessageCallback)
	...
	public void recoverAbnormally() {
		log.info(">>>>>>>>>>>>>>>CommitLog.recoverAbnormally>>>>>>>>");
		...
		if (!mapedFiles.isEmpty()) {
			...
			while (true) {
				DispatchRequest dispatchRequest = this.checkMessageAndReturnSize(byteBuffer, checkCRCOnRecover);
				int size = dispatchRequest.getMsgSize();
				log.info(">>>>>>>>>>>>>>>CommitLog.recoverAbnormally>>>>>>>>msgsize:" + size);
				// Normal data
				if (size > 0) {
					mapedFileOffset += size;
	...
	public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
		log.info(">>>>>>>>>>>>>>>>CommitLog.putMessage>>>>>>>>>>msg:");
		...
		if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
			...
			if (msg.isWaitStoreMsgOK()) {
				...
				log.info(">>>>>>>>>>>>>>>>>>>>>GroupCommitService request:" + request);
	...
	class FlushRealTimeService extends FlushCommitLogService {
		...
		public void run() {
			...
			while (!this.isStoped()) {
				...
				try {
					...
					boolean flushCommitLogALLFinshed = CommitLog.this.mapedFileQueue.commit(flushPhysicQueueLeastPages);
					// CommitLog.log.info(">>>>>>>>>>>>>FlushRealTimeService.run >>>>>>>>called mapedFileQueue.commit() to flush disk.>>>flushCommitLogALLFinshed:"
					// +flushCommitLogALLFinshed);
		...
		private void printFlushProgress() {
			CommitLog.log
					.info(">>>>>>>>>>>>>FlushRealTimeService.printFlushProgress>>>how much disk fall behind memory, "

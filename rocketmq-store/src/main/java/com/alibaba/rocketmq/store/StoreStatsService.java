...
public class StoreStatsService extends ServiceThread {
	...
	private String getGetTransferedTps(int time) {
		...
		try {
			...
			if (this.transferedMsgCountList.size() > time) {
				CallSnapshot lastBefore = this.transferedMsgCountList.get(this.transferedMsgCountList.size()
						- (time + 1));
				result += CallSnapshot.getTPS(lastBefore, last);
			}
	...
	public Map<String, ConcurrentHashMap<String, AtomicLong>> getGetMessageConsumerGroupTimesTotal() {
		return getMessageConsumerGroupTimesTotal;
	}
	...
	public AtomicLong getSingleGetMessageConsumerGroupTotal(String consumerGroup, String topic) {
		ConcurrentHashMap<String, AtomicLong> rs = getMessageConsumerGroupTimesTotal.get(consumerGroup);
		if (null == rs) {
			rs = new ConcurrentHashMap<String, AtomicLong>();
			rs.put(topic, new AtomicLong(0));
			getMessageConsumerGroupTimesTotal.put(consumerGroup, rs);
		} else {
			rs.putIfAbsent(topic, new AtomicLong(0));
		}

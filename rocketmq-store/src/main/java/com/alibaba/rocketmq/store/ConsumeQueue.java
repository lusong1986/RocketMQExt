...
public class ConsumeQueue {
				...
				try {
					...
					if (targetOffset != -1) {
						...
						if (leftIndexValue == -1) {
							...
							offset = Math.abs(timestamp - leftIndexValue) > Math.abs(timestamp - rightIndexValue) ? rightOffset
									: leftOffset;
						}
					}
				...
				try {
					...
					for (int i = 0; i < result.getSize(); i += ConsumeQueue.CQStoreUnitSize) {
						...
						if (offsetPy >= phyMinOffset) {
							...
							log.info(">>>>>>>>>>>correctMinOffset>>>>>>>>>compute logics min offset: "
									+ this.getMinOffsetInQuque() + ", topic: " + this.topic + ", queueId: "
									+ this.queueId);
							break;
						}
					}
	...
	public void putMessagePostionInfoWrapper(long offset, int size, long tagsCode, long storeTimestamp, long logicOffset) {
		log.info(">>>>>>>>>>>>>>>>ConsumeQueue.putMessagePostionInfoWrapper>>>offset:" + offset + ",logicOffset:"
				+ logicOffset + ",size:" + size + ",tagsCode:" + tagsCode);
	...
	private boolean putMessagePostionInfo(final long offset, final int size, final long tagsCode, final long cqOffset) {
		log.info(">>>>>>>>>>>>>>>>ConsumeQueue.putMessagePostionInfo>>>offset:" + offset + ",cqOffset:" + cqOffset
				+ ",size:" + size + ",tagsCode:" + tagsCode);
		...
		if (mapedFile != null) {
			...
			log.info(">>>>>>>>>>>>>>>>ConsumeQueue.putMessagePostionInfo>>>mapedFile:" + mapedFile.getFileName()
					+ ",getCommittedPosition:" + mapedFile.getCommittedPosition() + ">>>>>>>>expectLogicOffset:"
					+ expectLogicOffset);
			...
			log.info(">>>>>>>>>>>>>>>>ConsumeQueue.putMessagePostionInfo>>> this.maxPhysicOffset:"

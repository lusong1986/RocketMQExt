...
public class EndTransactionProcessor implements NettyRequestProcessor {
	...
	private MessageExtBrokerInner endMessageTransaction(MessageExt msgExt) {
		...
		TopicFilterType topicFilterType = (msgInner.getSysFlag() & MessageSysFlag.MultiTagsFlag) == MessageSysFlag.MultiTagsFlag ? TopicFilterType.MULTI_TAG
	...
	public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request)
		...
		logTransaction.info(">>>>>>>>>>>>>>>>>>>>>>>>>EndTransactionProcessor.processRequest:" + request);
		...
		if (requestHeader.getFromTransactionCheck()) {
			...
			case MessageSysFlag.TransactionNotType: {
				int checkTimes = TransactionPrepareMsgRecoder.getCheckTimes(requestHeader.getCommitLogOffset())
						.incrementAndGet();
				if (checkTimes >= TransactionPrepareMsgRecoder.MAX_CHECK_PREPARE_MSG_TIMES) {
					logTransaction
							.info(">>>>>>>>>>>>>>>>removePrepareMsgByLogOffset after check 10 times , it's pending status , prepare commitLogOffset:"
									+ requestHeader.getCommitLogOffset());
					TransactionPrepareMsgRecoder.removePrepareMsgRecord(requestHeader.getCommitLogOffset());
				}
		...
		// 正常提交回滚
			...
			case MessageSysFlag.TransactionNotType: {
				logTransaction
						.warn(">>>>>>>>>>>>>>>fromTransactionCheck is false. the producer[{}] end transaction in sending message,  and it's pending status.\n"//
								+ "RequestHeader: {} Remark: {}",//
								RemotingHelper.parseChannelRemoteAddr(ctx.channel()), //
								requestHeader.toString(),//
								request.getRemark());
			...
			case MessageSysFlag.TransactionRollbackType: {
				logTransaction
						.warn("################>>>>>>>>>>>>>TransactionRollbackType>>>>>>the producer[{}] end transaction in sending message, rollback the message.\n"//
								+ "RequestHeader: {} Remark: {}",//
								RemotingHelper.parseChannelRemoteAddr(ctx.channel()), //
								requestHeader.toString(),//
								request.getRemark());
		...
		logTransaction.info(">>>>>>>>>>>>>>>> removePrepareMsgByLogOffset :" + requestHeader.getCommitLogOffset());
		TransactionPrepareMsgRecoder.removePrepareMsgRecord(requestHeader.getCommitLogOffset());

		final MessageExt msgExt = this.brokerController.getMessageStore().lookMessageByOffset(
				requestHeader.getCommitLogOffset());
		if (msgExt != null) {
			// 校验Producer Group
			...
			if (msgExt.getQueueOffset() != requestHeader.getTranStateTableOffset()) {
				logTransaction.warn(">>>>>>>>>>>>>>>> getQueueOffset not equal");
				response.setCode(ResponseCode.SYSTEM_ERROR);
			...
			logTransaction.info(">>>>>>>>>>>>>>>>>>>>>>>>>EndTransactionProcessor.putMessage :" + msgInner);
			...
			if (putMessageResult != null) {
				logTransaction.info(">>>>>>>>>>>>>>>>>>>>>>>>>EndTransactionProcessor.putMessageResult :"
						+ putMessageResult);
		...
		logTransaction.info(">>>>>>>>>>>>>>>>>>>>>>>>>EndTransactionProcessor.response :" + response);
		return response;
	}
}

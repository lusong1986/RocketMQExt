...
public class Broker2Client {
	...
	public void checkProducerTransactionState(//
		...
		log.info(">>>>>>>>>>>>>>>>>>>>>>>>>Broker2Client.checkProducerTransactionState requestHeader:" + requestHeader);
			...
			for (Channel channel : channelInfoTable.keySet()) {
				...
				if (version >= MQVersion.Version.V3_0_7_SNAPSHOT.ordinal()) {
					try {
						...
						log.info("[reset-offset] reset offset success. topic={}, group={}, clientId={}", new Object[] {
								topic, group, channelInfoTable.get(channel).getClientId() });
					} catch (Exception e) {
						log.error("[reset-offset] reset offset exception. topic={}, group={}", new Object[] { topic,
								group }, e);
					}
				} else {
	...
	public RemotingCommand getConsumeStatus(String topic, String group, String originClientId) {
		...
		for (Channel channel : channelInfoTable.keySet()) {
			...
			if (version < MQVersion.Version.V3_0_7_SNAPSHOT.ordinal()) {
				...
				// originClientId 进行处理
				try {
					RemotingCommand response = this.brokerController.getRemotingServer().invokeSync(channel, request,
							5000);

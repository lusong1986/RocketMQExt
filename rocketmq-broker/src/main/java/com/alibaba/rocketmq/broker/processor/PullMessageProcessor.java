...
public class PullMessageProcessor implements NettyRequestProcessor {
	...
	private RemotingCommand processRequest(final Channel channel, RemotingCommand request, boolean brokerAllowSuspend)
		...
		log.debug(">>>>>>>>>>>>>>>>>>PullMessageProcessor>>>>>>>>>>>subscriptionGroupConfig:" + subscriptionGroupConfig);
		...
		log.debug(">>>>>>>>>>>>>>>>>>PullMessageProcessor>>>>>>>>>>>hasSuspendFlag:" + hasSuspendFlag
				+ ",hasCommitOffsetFlag:" + hasCommitOffsetFlag + ",hasSubscriptionFlag:" + hasSubscriptionFlag
				+ ",suspendTimeoutMillisLong:" + suspendTimeoutMillisLong);
		...
		log.debug(">>>>>>>>>>>>>>>>>>PullMessageProcessor>>>>>>>>>>>topicConfig:" + topicConfig);
		...
		if (hasSubscriptionFlag) {
			...
			log.debug(">>>>>>>>>>>>>>>>>>PullMessageProcessor>>>>>>>>>>>consumerGroupInfo:" + consumerGroupInfo);
			...
			log.debug(">>>>>>>>>>>>>>>>>>PullMessageProcessor>>>>>>>>>>>subscriptionData:" + subscriptionData);
		...
		if (getMessageResult != null) {
			...
			case FOUND:
				...
				if (this.hasConsumeMessageHook()) {
					...
					final SocketAddress storeHost = new InetSocketAddress(brokerController.getBrokerConfig()
							.getBrokerIP1(), brokerController.getNettyServerConfig().getListenPort());
